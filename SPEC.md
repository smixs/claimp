...
- BPM, тональность (Key), анализ бита, cue-точки, метки на волне, зацикливание.
...
├──┼───┼─────────────────┼───────────────┼─────┼───────┤
│● │ 1 │ Future Garage…  │ Dj Antiz & Z… │2014 │ 32:07 │ СТРОКА: 26, одна строка на трек, 12 pt
...
Ширины колонок (pt): `●` 22 (фикс), `#` 26 (фикс, правое выравнивание), `Название` min 130 / ideal 190
(растёт), `Исполнитель` min 100 / ideal 140 (растёт), `Год` 44 (фикс, правое), `Длительность` 52
(фикс, правое). Сумма минимумов 374 pt плюс поля и скроллер - влезает в минимальные 420 pt ширины.
...
Заголовки колонок 11 pt, строки 13 pt, вторая строка описания трека **не делается** (одна строка на трек, DECISIONS 09:55).
...
15. **Обложка** - из тега текущего трека; нет обложки - пустой прямоугольник фона `#282828` без
    иконки-заглушки.
16. **Ошибки** - fail fast: не открылся файл при воспроизведении - алерт с путём и причиной,
