...
| 3 | Плейлист колонками с заголовками: лампочка, номер, Title, Artist, Year, Length | DECISIONS 09:55 |
...
├──────────────────────────────────────────────────────┤
│● │ # │ Title           │ Artist        │Year │Length │ ЗАГОЛОВКИ: 20, кликабельные, 10 pt
├──┼───┼─────────────────┼───────────────┼─────┼───────┤
...
тональность): `●` 16, `#` 22 (правое), `Title` 130 (растёт), `Artist` 100 (растёт),
`Year` 32 (правое), `Length` 44 (правое), `kbps` 52 (правое), `BPM` 48 (правое, без дробной части),
...
(у лампочки минимум - сама лампочка), не влезшее содержимое обрезается как в Title/Artist,
...
Лишнюю ширину окна делят между собой только Title и Artist. Значение берётся по приоритету
...
Цифры в `#`, `Year`, `Length`, в подписях времени под волной и в статусной строке - `monospacedDigitSystemFont`.
