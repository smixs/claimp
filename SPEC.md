...
Колонки и ширины (pt, решение владельца 2026-09-15 17:57 - добавлены битрейт, BPM, тональность):
`●` 16 (фикс), `#` 22 (фикс, правое), `Название` min 92 / ideal 130 (растёт), `Исполнитель` min 70 /
ideal 100 (растёт), `Год` 32 (фикс, правое), `Длит.` 44 (фикс, правое), `kbps` 52 (фикс, правое),
`BPM` 48 (фикс, правое, без дробной части), `Key` 44 (фикс, код Camelot, подсказка «8A · Am»).
Сумма минимумов 420 pt - ровно минимальная ширина окна. Значение берётся по приоритету
...
| Анализ | Автоанализ | вкл/выкл | вкл |
